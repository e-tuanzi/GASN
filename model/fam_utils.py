...
def extract_features(feature_model, image, boxes, feat_map_keys=['map3', 'map4'], exemplar_scales=[0.9, 1.1]):
    ...
    for ix in range(0, N):
        ...
        for keys in feat_map_keys:
            ...
            boxes_scaled[:, 1:3] = torch.floor(boxes_scaled[:, 1:3])  # 向下取整
            boxes_scaled[:, 3:5] = torch.ceil(boxes_scaled[:, 3:5])  # 向上取整
            ...
            # print("examples_features", examples_features.shape)
