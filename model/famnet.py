...
from model.fam_utils import MAPS, Scales, Transform, extract_features
...
class FamNet:
    def __init__(self, model_path, device):
        ...
        if device == "cuda":
            ...
            self.regressor.cuda()
            self.regressor.load_state_dict(torch.load(model_path))
        else:
            self.regressor.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))
    ...
    def predict(self, image, boxes):
        if self.device == "cuda":
            ...
            boxes = boxes.cuda()
        with torch.no_grad():
            features = extract_features(self.resnet50_conv, image, boxes, MAPS, Scales)
        with torch.no_grad():
